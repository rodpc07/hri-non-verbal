...
moveit::planning_interface::MoveGroupInterface::Plan exagerateTrajectory(moveit::planning_interface::MoveGroupInterface::Plan plan, Eigen::Vector3d xyz)
{
    ...
    // moveit_msgs::MotionSequenceRequest request;
    // request.header.stamp = ros::Time::now();
    // request.robot_trajectory.resize(1);
    // request.robot_trajectory[0].joint_trajectory.joint_names = joint_names;
    // request.robot_trajectory[0].joint_trajectory.points = trajectory_points;
    ...
    for (int i = 1; i < num_waypoints - 1; i++)
    {
        ...
        kinematic_state->setFromIK(kinematic_model->getJointModelGroup("manipulator"), target_pose);
...
int main(int argc, char **argv)
{
    ...
    // Modify Trajectory
    Eigen::Vector3d xyz = Eigen::Vector3d(0.0, 0.0, -0.1);

    moveit::planning_interface::MoveGroupInterface::Plan modified_plan = exagerateTrajectory(my_plan, xyz);
    ...
    do
    {
        ...
        case 4:
            ...
            modified_plan = exagerateTrajectory(my_plan, xyz);
            break;
        case 5:
            cout << "X Y Z:";
            cin >> str1 >> str2 >> str3;
